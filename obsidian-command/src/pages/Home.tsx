...
function FloatingOrb({ className }: { className?: string }) {
  ...
  return (
    <motion.div
      className={cn(
        'pointer-events-none absolute rounded-full blur-3xl opacity-40 dark:opacity-30',
        className,
      )}
      aria-hidden
      animate={
...
function VoiceEqualizer({ className }: { className?: string }) {
  const heights = ['h-8', 'h-14', 'h-10', 'h-16', 'h-9', 'h-12'];
  return (
    <div
      className={cn('flex items-end justify-center gap-1.5 h-16', className)}
      aria-hidden
    >
      {heights.map((h, i) => (
        <div
          key={i}
          className={cn(
            'w-2 rounded-full bg-primary/80 dark:bg-primary/90 marketing-eq-bar',
            h,
          )}
        />
      ))}
  ...
  return (
    <div className="min-h-screen bg-background text-on-surface overflow-x-hidden">
      <div className="fixed inset-0 pointer-events-none overflow-hidden">
        <FloatingOrb className="left-[-10%] top-[15%] h-72 w-72 bg-primary/30" />
        <FloatingOrb className="right-[-5%] top-[40%] h-96 w-96 bg-secondary-container/40 dark:bg-primary/20" />
      </div>
      ...
      <main className="relative mx-auto max-w-6xl px-4 pb-24 sm:px-6">
        ...
        <footer className="border-t border-outline-variant/10 pt-10 text-center text-xs text-on-surface-variant">
          <p>Sonix — Obsidian Command · Voice agents for teams that ship.</p>
        </footer>
      </main>
    </div>
  );
