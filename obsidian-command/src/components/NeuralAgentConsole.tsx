  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
      <svg ref={waveRef} className="w-full h-full p-4" viewBox="0 0 100 100" preserveAspectRatio="none">
        <path d="M0,50 Q25,50 50,50 T100,50" fill="none" stroke="#f97316" strokeWidth="0.5" />
        <path d="M0,50 Q25,50 50,50 T100,50" fill="none" stroke="#f97316" strokeWidth="0.3" opacity="0.5" />
      </svg>
    </div>
  );
});
      ...
      <AnimatePresence>
        {isActive && (
          <motion.div 
            ...
            style={{
              boxShadow: "inset 0 0 100px rgba(249,115,22,0.1)"
            }}
          >
            <motion.div 
              animate={{ opacity: [0.1, 0.3, 0.1] }}
              transition={{ duration: 2, repeat: Infinity }}
              className="absolute inset-0 bg-radial-at-c from-orange-500/5 to-transparent blur-3xl"
            />
          </motion.div>
        )}
      </AnimatePresence>
      ...
      >
        ...
        >
          <AnimatePresence>
            {chaosMode !== 'neutral' && (
              <motion.div 
                ...
                exit={{ opacity: 0 }}
                className="absolute top-6 px-4 py-1.5 rounded-full border border-orange-500 bg-orange-500/10 text-orange-500 text-[8px] font-black uppercase tracking-widest z-50 flex items-center gap-2"
              >
          ...
          <div 
            ...
            className={`relative size-60 rounded-full cursor-pointer group transition-all duration-1000 ${isActive ? 'scale-105 shadow-[0_0_80px_rgba(249,115,22,0.15)]' : 'hover:scale-105 opacity-80'}`}
          >
            {[...Array(3)].map((_, i) => (
              <motion.div
                key={i}
            ...
            <div className={`absolute inset-4 rounded-full bg-[#050608] border-2 border-zinc-800 flex items-center justify-center overflow-hidden transition-all duration-700 ${isActive ? 'border-orange-500 ring-20 ring-orange-500/5 shadow-2xl shadow-orange-500/20' : ''}`}>
               <div className="absolute inset-0 opacity-10 bg-[radial-gradient(circle_at_center,orange_0%,transparent_70%)]" />
               <Activity className={`size-16 transition-all duration-700 ${isActive ? 'text-orange-500 scale-125' : 'text-zinc-700'}`} />
          ...
          <div className="text-center space-y-3">
            <h2 className="text-xl font-black text-white tracking-tighter">
              {agent?.name ? <span className="text-orange-500 uppercase">{agent.name}</span> : 'ASTRA'} 2.0
            </h2>
            <div className="flex flex-col items-center gap-2">
               <span className="flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest text-zinc-500">
                 <Shield className="size-2.5 text-orange-500" /> Secure Encryption
               </span>
               <span className="flex items-center gap-1.5 text-[8px] font-black uppercase tracking-widest text-zinc-500">
                 <Languages className="size-2.5 text-orange-500" /> Vernacular Active
               </span>
            </div>
          </div>
        </motion.div>
        ...
        <AnimatePresence>
          {showControlRoom && (
            ...
            >
                ...
                <div className="flex items-center justify-between mb-4 shrink-0 px-2">
                  ...
                  <div className="flex items-center gap-2">
                    {!isActive && transcript.length > 0 && (
                      <span className="text-[7px] font-black text-white bg-zinc-800 px-2 py-0.5 rounded uppercase flex items-center gap-1">
                        <Info className="size-2 text-orange-500" /> Session Frozen
                      </span>
                    )}
                    <div className={`size-1.5 rounded-full ${isActive ? 'bg-orange-500 animate-pulse' : 'bg-zinc-700'}`} />
                   ...
                   <AnimatePresence mode="wait">
                     {isConnecting ? (
                        <motion.div 
                          ...
                          animate={{ opacity: 1 }}
                          exit={{ opacity: 0 }}
                          className="absolute inset-0 flex flex-col items-center justify-center bg-zinc-950/20 backdrop-blur-sm z-50 rounded-[32px] p-12"
                        >
                           <div className="w-full max-w-[280px] space-y-4">
                              <div className="flex items-center justify-between">
                                 <span className="text-[10px] font-black text-orange-500 uppercase tracking-[0.3em]">CALIBRATING_DNA</span>
                                 <span className="text-[10px] font-mono text-orange-500/50">84%</span>
                              </div>
                              <div className="h-1 w-full bg-zinc-900 overflow-hidden rounded-full border border-zinc-800">
                                 <motion.div 
                                   initial={{ x: '-100%' }}
                                   animate={{ x: '0%' }}
                                   transition={{ duration: 2.5, ease: "easeInOut" }}
                                   className="h-full w-full bg-linear-to-r from-orange-600 to-orange-400 shadow-[0_0_15px_rgba(249,115,22,0.4)]"
                                 />
                              </div>
                       ...
                       transcript.map((m, i) => (
                         ...
                         >
                            <div className={`max-w-[90%] p-3 rounded-2xl text-xs transition-all duration-500 ${
                              m.speaker === 'system' ? 'bg-orange-500/5 text-orange-400 font-mono text-[9px] uppercase border border-orange-500/10' :
                              m.speaker === 'user' ? 'bg-zinc-900 border border-zinc-800 text-zinc-400' : 
                              'bg-orange-500/10 border border-orange-500/20 text-white shadow-[0_0_20px_rgba(249,115,22,0.05)]'
                            }`}>
                              {m.text}
                              {m.lang && (
                       ...
                       <motion.div 
                         ...
                         animate={{ opacity: 1 }}
                         key="idle"
                         className="absolute inset-0 flex flex-col items-center justify-center space-y-6 bg-zinc-950/40 backdrop-blur-sm z-50 rounded-3xl"
                       >
                         <div className="size-20 rounded-full border border-orange-500/20 bg-orange-500/5 flex items-center justify-center relative">
                            <motion.div 
                         ...
                         <div className="text-center">
                            <h3 className="text-sm font-black text-white uppercase tracking-widest mb-1">Link Status: Ready</h3>
                            <p className="text-[10px] text-zinc-500 font-medium max-w-[200px]">Configuration Loaded. Awaiting Neural Link Initialization.</p>
                         </div>
                         <button 
                           onClick={() => {
                           ...
                           className="px-8 py-3 rounded-full bg-orange-500 text-black text-[10px] font-black uppercase tracking-widest hover:bg-white transition-all shadow-xl shadow-orange-500/20 active:scale-95"
                         >
              ...
              <div className="flex-1 bg-zinc-950/50 border border-zinc-800/50 rounded-[32px] p-5 flex gap-4 min-h-[160px]">
                 {/* LIVE SUMMARY TICKER */}
                 <div className="flex-[1.5] flex flex-col gap-2">
                   <div className="flex items-center gap-2 mb-1">
                     <Sparkles className="size-3 text-orange-500" />
                     <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest">Live Summary Feed</span>
                   </div>
                   <div className="space-y-2 overflow-hidden">
                     <AnimatePresence mode="popLayout">
                       {summary.map((item, idx) => (
                         ...
                         >
                           <span className="font-mono text-orange-500/50 shrink-0">[{item.time}]</span>
                           <span className="text-zinc-400 font-medium leading-tight">{item.text}</span>
                         </motion.div>
                       ))}
                     </AnimatePresence>
                   </div>
                 ...
                 <div className="flex-1 flex flex-col items-center justify-center border-l border-zinc-800/50 pl-4 py-2 relative">
                    <span className="absolute top-0 right-0 text-[8px] font-black text-zinc-600 uppercase tracking-widest">Confidence</span>
                    <div className="relative size-24">
                      ...
                      <svg className="size-full overflow-visible" viewBox="0 0 100 100">
                        ...
                        <motion.circle 
                          cx="50" cy="50" r="45" 
                          fill="none" 
                          stroke={propensity > 70 ? "#10b981" : propensity > 40 ? "#f97316" : "#ef4444"} 
                          strokeWidth="8" 
                          strokeLinecap="round"
                      ...
                      <div className="absolute inset-0 flex flex-col items-center justify-center pt-2">
                        <span className="text-xl font-black text-white tabular-nums leading-none">{Math.round(propensity)}%</span>
                        <span className="text-[7px] font-bold text-zinc-500 uppercase tracking-tighter">Propensity</span>
                      </div>
                    </div>
                 </div>
              </div>
        ...
        <AnimatePresence>
          {showControlRoom && (
            ...
            >
              ...
              <div className="flex-1 bg-zinc-950 border border-zinc-800 rounded-[32px] p-6 flex flex-col gap-5 relative overflow-hidden">
                <div className="absolute top-0 right-0 size-32 bg-orange-500/5 blur-[60px] rounded-full" />
                
                <div className="flex items-center justify-between shrink-0">
                  <span className="text-[10px] font-black uppercase tracking-[0.4em] text-orange-500 flex items-center gap-2">
                    <Sparkles className="size-3" /> Experiment Lab
                  </span>
                  <Settings className="size-3 text-zinc-700 animate-spin-slow" />
                </div>

                <div className="space-y-5 overflow-y-auto no-scrollbar pb-4 pr-1">
                  {/* SECTOR A: EMOTION DNA */}
                  <div className="space-y-2">
                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest block px-1">Response Tone</span>
                    <div className="p-1 bg-zinc-900/40 rounded-2xl border border-zinc-800 flex gap-1">
                      {['empathetic', 'neutral', 'firm'].map(e => (
                        <button
                          ...
                          className={`flex-1 py-1.5 rounded-xl text-[8px] font-black uppercase transition-all ${
                            emotion === e ? 'bg-orange-500 text-black shadow-lg shadow-orange-500/20' : 'text-zinc-500 hover:text-zinc-300'
                          }`}
                        >
                          {e}
                        </button>
                  ...
                  <div className="space-y-2">
                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest block px-1">Neural Layers</span>
                    <div className="grid grid-cols-2 gap-2">
                      <button 
                        onClick={() => setMemoryEnabled(!memoryEnabled)}
                        className={`flex flex-col gap-1 p-3 rounded-2xl border transition-all text-left ${
                          memoryEnabled ? 'bg-orange-500/10 border-orange-500/30' : 'bg-black/40 border-zinc-900'
                        }`}
                      >
                         <div className={`size-1.5 rounded-full ${memoryEnabled ? 'bg-orange-500' : 'bg-zinc-700'}`} />
                         <span className="text-[9px] font-black uppercase text-white tracking-tighter leading-none mt-1">Memory</span>
                         <span className="text-[7px] text-zinc-500 font-bold uppercase tracking-tight">Contextual Sync</span>
                      </button>
                      
                      <button 
                        onClick={() => setGuardrailsEnabled(!guardrailsEnabled)}
                        className={`flex flex-col gap-1 p-3 rounded-2xl border transition-all text-left ${
                          guardrailsEnabled ? 'bg-orange-500/10 border-orange-500/30' : 'bg-black/40 border-zinc-900'
                        }`}
                      >
                         <div className={`size-1.5 rounded-full ${guardrailsEnabled ? 'bg-orange-500' : 'bg-zinc-700'}`} />
                         <span className="text-[9px] font-black uppercase text-white tracking-tighter leading-none mt-1">Safety</span>
                         <span className="text-[7px] text-zinc-500 font-bold uppercase tracking-tight">Compliance Gate</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2">
                    <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest block px-1">Voice & Language DNA</span>
                    <div className="space-y-2">
                      <div className="grid grid-cols-2 gap-2">
                        <select 
                          value={selectedVoice}
                          onChange={(e) => setSelectedVoice(e.target.value)}
                          className="w-full bg-zinc-950 border border-zinc-800 rounded-xl p-2.5 text-[9px] font-bold text-white outline-none focus:border-orange-500 transition-all appearance-none cursor-pointer"
                        >
                      ...
                      <div className="flex gap-2">
                        <button 
                          ...
                          className={`flex-1 py-2 rounded-xl border text-[8px] font-black uppercase transition-all ${
                            vocalSpeed === 'fast' ? 'bg-white text-black border-white' : 'bg-black border-zinc-800 text-zinc-500'
                          }`}
                        >
                        ...
                        <button 
                          ...
                          className={`flex-1 py-2 rounded-xl border text-[8px] font-black uppercase transition-all ${
                            inferenceLatency === 'low' ? 'border-orange-500/50 text-orange-500 bg-orange-500/5' : 'bg-black border-zinc-800 text-zinc-500'
                          }`}
                        >
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center justify-between px-1">
                      <span className="text-[8px] font-black text-zinc-500 uppercase tracking-widest block">Stress Testing</span>
                      <div className="flex gap-1">
                         <div className="size-1 rounded-full bg-orange-500 animate-pulse" />
                         <span className="text-[6px] text-zinc-600 font-bold uppercase">Ready</span>
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-2">
                       <button 
                        onClick={() => setChaosMode('interrupt')}
                        className="py-2.5 rounded-xl border border-zinc-800 bg-zinc-900/30 text-zinc-400 text-[8px] font-black uppercase hover:bg-red-500 hover:text-white hover:border-red-600 transition-all"
                       >
                         Interrupt
                       </button>
                       <button 
                        onClick={() => setChaosMode('lang-flip')}
                        className="py-2.5 rounded-xl border border-zinc-800 bg-zinc-900/30 text-zinc-400 text-[8px] font-black uppercase hover:bg-orange-500 hover:text-black hover:border-orange-600 transition-all"
                       >
                         Lang Flip
                       </button>
                    </div>
                ...
                <div className="mt-auto pt-4 border-t border-zinc-800/50 flex flex-col gap-3">
                   ...
                   <div className="flex items-center gap-2">
                     <Brain className="size-3 text-orange-500/50" />
                     <span className="text-[8px] font-black uppercase text-zinc-500 tracking-[0.2em]">Neural_Status</span>
                   </div>
                   <p className="text-[9px] font-mono text-orange-400/60 uppercase px-1 line-clamp-1">
                     {reasoning}
                   </p>
                </div>
              </div>
